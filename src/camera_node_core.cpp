...
//     //TODO: Replace this with VizionSDK properties
...
void CameraNode::generate_mat_mappings()
{
    ...
    for (auto &[cvtype, prefix] :
         {std::tuple<int, std::string>{CV_8U, "8U"},
          ...
          std::tuple<int, std::string>{CV_8UC2, "8UC2"}})
    {
...
CameraNode::CameraNode(const rclcpp::NodeOptions &options) : Node("camera_node", options)
{
    ...
    if(deviceCount==1){
        CamL = VxInitialCameraDevice(0);
        VxOpen(CamL);
    }
    if(deviceCount==2){
        std::shared_ptr<VxCamera> cam0 = VxInitialCameraDevice(0);
        std::shared_ptr<VxCamera> cam1 = VxInitialCameraDevice(1);
        VxOpen(cam0);
        VxOpen(cam1);
        std::string hwId0;
        std::string hwId1;
        ...
        VxGetOSPProfileConfig(CamL, profile);
        VxLoadProfileConfig(CamL, profile);
        VxLoadProfileConfig(CamR, profile);
        VxActivateProfileStreaming(CamL);
        VxActivateProfileStreaming(CamR);
        VxActivateProfileImageProcessing(CamL);
        VxActivateProfileImageProcessing(CamR);
    }

    VxStartStreaming(CamL);
    if(deviceCount==2){
    ...
    // }
    //TODO: Set camera format

    declare_parameter<double>("camera_refresh_rate", 30.0);
    declare_parameter<double>("camera_info_rate", 0.1);
    declare_parameter<std::vector<int64_t>>("camera_frame_dim", std::vector<int64_t>{640, 480});
    ...
    camera_frame_clock = create_wall_timer(
        ...
        std::bind(&CameraNode::camera_frame_callback_stereo, this));
    }
    else{
        camera_frame_clock = create_wall_timer(
        std::chrono::duration<double>(1 / get_parameter("camera_refresh_rate").as_double()),
